...
chunks their text using a selectable strategy, generates Gemini embeddings, and
stores vectors in Qdrant. The conversational API retrieves relevant context
...
\begin{tabularx}{\textwidth}{L{3.4cm} L{4.8cm} Y}
    ...
    Tokeniser         & NLTK Punkt              & Sentence-based chunking \\
    \bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \texttt{fixed} or \texttt{sentence} chunking.
    \item FastAPI validates the extension and extracts text.
    \item The chunker splits extracted text with the selected strategy.
    \item Gemini generates a vector for each chunk using the
    \texttt{retrieval\_document} task type.
    \item Qdrant stores vectors with document ID, filename, and chunk text.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Otherwise, Gemini embeds the query using the
    \texttt{retrieval\_query} task type.
...
\begin{tabularx}{\textwidth}{L{4.7cm} Y}
    ...
    \path{app/services/chunker.py} & Provides fixed-size overlapping chunks
    and NLTK sentence-group chunks. \\
    ...
    upserts embedded chunks, and retrieves the top five matches using cosine
    similarity. \\
...
\begin{tabularx}{\textwidth}{L{2.5cm} L{5cm} Y}
    ...
    \midrule
    \texttt{fixed} & 500 characters, 50-character overlap &
    Dense or irregular text \\
    \texttt{sentence} & Up to 5 NLTK-tokenised sentences &
    Structured prose and FAQs \\
    \bottomrule
\end{tabularx}

\subsection{Vector retrieval}

Qdrant stores one 768-dimensional Gemini embedding for each document chunk.
...
matching chunks to Gemini as grounded context.
...
\begin{tabularx}{\textwidth}{L{2.5cm} L{2.5cm} Y}
    ...
    \texttt{strategy} & String & Required: \texttt{fixed} or
    \texttt{sentence} \\
    \bottomrule
\end{tabularx}
...
\begin{lstlisting}[language=bash]
curl -X POST http://localhost:8000/api/v1/ingest \
  -F "file=@test_doc.txt" -F "strategy=fixed"

curl -X POST http://localhost:8000/api/v1/ingest \
  -F "file=@test_doc.txt" -F "strategy=sentence"
\end{lstlisting}
...
docker compose exec postgres \
  ...
  -c "SELECT * FROM bookings ORDER BY created_at DESC;"
\end{lstlisting}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Selectable chunking}: callers can choose predictable
    character windows or sentence-aware grouping based on document shape.
\end{itemize}
